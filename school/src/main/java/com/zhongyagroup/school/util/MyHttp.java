...
public class MyHttp {
	...
	private class http_soap extends Thread
	{
		public void run()
		{
			try
			{
				Log.i("MyHttp","请求的网址：" + vmserviceurl);
				Log.i("MyHttp","发送出去的数据为：" + vmservicepara);
				...
				Log.i("MyHttp","收到回复的数据为：" + (rtn.length()==0?"(空)":rtn));
	...
	private String httpUrlConnection(String pathUrl, String requestString)
	{
		...
		try
		{
			...
			{// 连接成功
				...
				Log.e(TAG, "返回的错误代码：" + responseCode + "[" + pathUrl + "]" + "[" + requestString + "]");
			}
			httpConn.disconnect();
