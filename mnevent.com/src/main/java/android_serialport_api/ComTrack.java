...
public class ComTrack {
    // 是否测试模式
    public static boolean iTestMode = true;
    ...
    private String sPort = "/dev/ttyUSB1";
    ...
    public void openSerialPort() {
        ...
        if (!isOpen) {
            try {
                ...
                Log.i("zlc",portName + " 打开串口成功!");
            } catch (SecurityException e) {
                Log.e("zlc",portName + " 打开串口失败open fail" + e.getMessage());
            } catch (IOException e) {
                Log.e("zlc",portName + " 打开串口失败rw fail" + e.getMessage());
            }
        }
    ...
    public void reset(){
        ...
        Log.i("zlc","发送reset数据：" + bytesToHexString(fa_cmd,fa_cmd.length));
        ...
        if(rxByteArray == null){
            Log.i("zlc","接收到reset数据：空，等待了20秒");
            return;
        }

        // 哈哈，有数据了
        Log.i("zlc","接收到reset数据：" + bytesToHexString(rxByteArray,rxByteArray.length));
    ...
    public String[] vend_out_ind(int hang,int lie,int fen,String goodscode,String payway,String payinfo)
    {
        ...
        for(int i=1;i<=3;i++) {
            ...
            Log.i("zlc","发送数据：" + bytesToHexString(fa_cmd,fa_cmd.length)+";snindex="+snindex);
            ...
            // 哈哈，有数据了
            Log.i("zlc","接收到数据：" + bytesToHexString(rxByteArray,rxByteArray.length));
            ...
            if(rxByteArray.length == 5){
                ...
                if(status == 0) {
                    ...
                    if(outusetime < 1000){
                        Log.i("zlc","出货正常完成了,花费时间（毫秒）："+outusetime+";snindex="+snindex+";因为小于1000，则序号加1后再次发送");
                        // 少于1秒的，应该是序号重复了，那么要再次发送
                        snindex++;
                        if(snindex>250) snindex = 0;
                        continue;
                    } else {
                        Log.i("zlc","出货正常完成了,花费时间（毫秒）："+outusetime+";snindex="+snindex);
